...
# We need to replace `process_chat_in_background` completely.
import ast
import re

# We will just use regex or split to replace the process_chat_in_background and upload_file
# Actually, since I know the file exactly, I can just write the full new file string.

new_main_content = """import os
import tempfile
import asyncio
import json
...
def process_chat_in_background(task_id: str, filepath: str, source: str, use_llm: bool = False, llm_model: str = "qwen2.5:1.5b", file_hash: str = None, force_action: str = None):
    try:
        config = {
            "use_llm": use_llm,
            "llm_model": llm_model
        ...
        if is_stage_cached(file_hash, "04_embeddings"):
            ...
            text_msgs = compute_embeddings(df, task_id)
        ...
        semantic_stats = compute_semantic_stats(df, text_msgs, use_llm=use_llm, llm_model=llm_model, file_hash=file_hash)
        ...
        if filepath and os.path.exists(filepath):
            pass # Keep source file in checkpoints instead of removing temp
...
async def upload_file(
    ...
    valid_exts = {".json": "telegram", ".txt": "whatsapp", ".zip": "whatsapp"}
    ...
    background_tasks.add_task(
        ...
        task_id, filepath, source, use_llm_bool, llm_model, sha256_hash, force_action
    )
